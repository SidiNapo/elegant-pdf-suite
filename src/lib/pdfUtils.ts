...
// ==================== PPTX TO PDF - Using modular parser ====================
import { parsePPTX, renderPresentationToBytes } from './pptx';

export const pptToPDF = async (file: File): Promise<Uint8Array> => {
  // Check if it's a .ppt (binary) file
  const fileName = file.name.toLowerCase();
  if (fileName.endsWith('.ppt') && !fileName.endsWith('.pptx')) {
    throw new Error('Le format .ppt (PowerPoint 97-2003) n\'est pas supporté. Veuillez convertir votre fichier en .pptx.');
  }
  
  try {
    // Parse the PPTX file using the modular parser
    const presentation = await parsePPTX(file);
    
    // Log warnings if any
    if (presentation.warnings.length > 0) {
      console.warn('PPTX parsing warnings:', presentation.warnings);
    }
    
    // Render to PDF
    const pdfBytes = renderPresentationToBytes(presentation);
