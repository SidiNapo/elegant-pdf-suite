  ...
  return <AdminLayout title="Dashboard">
      ...
      delay: 0.4
        ...
        {isLoading ? <div className="text-center py-8 text-muted-foreground">
            Chargement...
          </div> : recentPosts.length > 0 ? <div className="space-y-4">
            {recentPosts.map(post => <Link key={post.id} to={adminRoutes.postsEdit(post.id)} className="flex items-center justify-between p-4 rounded-xl hover:bg-muted transition-colors">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <span className={`px-2 py-1 rounded-full text-xs font-medium ${post.is_published ? 'bg-green-500/10 text-green-500' : 'bg-yellow-500/10 text-yellow-500'}`}>
