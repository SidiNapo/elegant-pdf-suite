...
import { deletePages, downloadPDF, getPDFPageCount } from '@/lib/pdfUtils';
...
const DeletePages = () => {
  ...
  const handleDelete = async () => {
    ...
    try {
      const result = await deletePages(files[0], selectedPages);
      setResultPdf(result);
      setIsComplete(true);
    } catch (error) {
  ...
  const handleDownload = () => {
    if (resultPdf) {
      downloadPDF(resultPdf, 'modified.pdf');
    }
  };
